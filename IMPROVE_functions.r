...
checkIPTList <- function(informativeTerms) {
    multiIndx <- 0;
    multi <- list();
    parentsIndx <- 0;
    parents <- list();
                        ...
                        for(l in 1:length(tj)) {
                            if(rownames(ti)[k] %in% expandToParents(rownames(tj)[l])) {
                                if(i!=j && rownames(ti)[k]==rownames(tj)[l]) { ## top level differs but duplicated child
                                    ...
                                    multiIndx <- multiIndx+1;
                                    multi[[multiIndx]] <- list(upper1=upperT[1],
                                                                         upper2=upperT[2],
                                                                         child=rownames(ti)[k]);
    ...
    return(list(parents=parents,multi=multi));
}
...
improve_HPO <- function(data,priors,LOO,dtest=NULL) {
    ...
    probandIDs <- data$ID;
    ## subset of hpou setting usage of toplevel terms (names(informativePhenotypicTerms)) to have
    ## usage 0 if one or more specific child term is used
    ## retaining completeness of annotation for top-level terms lacking annotation to IPTs
...
## run improve() (applying pryFn) on all genes testing models for gene vs not-gene
